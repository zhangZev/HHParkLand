...
import com.zbar.lib.zxing.common.BitmapUtils;
import com.zbar.lib.zxing.decode.BitmapDecoder;
import com.zbar.lib.zxing.decode.CaptureActivityHandler;
import com.zbar.lib.zxing.view.ViewfinderView;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, View.OnClickListener {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case PARSE_BARCODE_SUC: // 解析图片成功
					...
					setResult(msg.obj.toString());
					break;
				case PARSE_BARCODE_FAIL:// 解析图片失败
					finish();
					Toast.makeText(this.activityReference.get(), "解析图片失败", Toast.LENGTH_SHORT).show();
					break;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.m_pic).setOnClickListener(this);
		findViewById(R.id.m_light).setOnClickListener(this);
...
//			break;
			...
			switch (requestCode) {
				case REQUEST_CODE:
					...
					if (cursor.moveToFirst()) {
						this.photoPath = cursor.getString(cursor.getColumnIndex(MediaColumns.DATA));
					}
					cursor.close();
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							Bitmap img = BitmapUtils.getCompressedBitmap(CaptureActivity.this.photoPath);
	...
	public void handleDecode(Result rawResult, Bitmap barcode, float scaleFactor) {
		...
		if ((this.source == IntentSource.NONE) && this.lastResult != null) { // 重新进行扫描
			restartPreviewAfterDelay(0L);
		}
	}
	...
	private void initCamera(SurfaceHolder surfaceHolder) {
		...
		try {
			...
			// RuntimeException.
			if (this.handler == null) {
				this.handler = new CaptureActivityHandler(this, this.decodeFormats, this.decodeHints,
	...
	public void onClick(View v) {
		if (v.getId() == R.id.m_back) {
			finish();
		}
		else if (v.getId() == R.id.m_pic) {
			// 打开手机中的相册
		...
		else if (v.getId() == R.id.m_light) {
			if (this.isFlashlightOpen) {
				this.cameraManager.setTorch(false); // 关闭闪光灯
				this.isFlashlightOpen = false;
			}
			else {
				this.cameraManager.setTorch(true); // 打开闪光灯
