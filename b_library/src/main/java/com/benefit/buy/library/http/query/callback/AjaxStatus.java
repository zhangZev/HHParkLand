...
public class AjaxStatus {
    ...
    /**
     * Close any opened inputstream associated with the response. Call this method when finish parsing the response of a
     * synchronous call.
     */
    public void close() {
    ...
    /**
     * Gets the http response code. Can be also be NETWORK_ERROR, AUTH_ERROR, or TRANSFORM_ERROR.
     * @return code
     */
    public int getCode() {
        return code;
    ...
    /**
     * Test if the response is expired against current time, given the expire duration in milliseconds. If the ajax
     * source is NETWORK, it's never considered expired.
    ...
    /**
     * Return the http response headers. Return values only when source is not from cache (source == NETWORK), returns
