...
public class WheelOptions<T> {
    ...
    public void setPicker(ArrayList<T> options1Items, ArrayList<ArrayList<T>> options2Items,
        ...
        this.wv_option1.setAdapter(new ArrayWheelAdapter(this.mOptions1Items, len));// 设置显示数据
        this.wv_option1.setCurrentItem(0);// 初始化时显示的数据
        // 选项2
        this.wv_option2 = (WheelView) this.view.findViewById(R.id.options2);
        if (this.mOptions2Items != null) {
            this.wv_option2.setAdapter(new ArrayWheelAdapter(this.mOptions2Items.get(0)));// 设置显示数据
        }
        this.wv_option2.setCurrentItem(this.wv_option1.getCurrentItem());// 初始化时显示的数据
        // 选项3
        this.wv_option3 = (WheelView) this.view.findViewById(R.id.options3);
        if (this.mOptions3Items != null) {
            this.wv_option3.setAdapter(new ArrayWheelAdapter(this.mOptions3Items.get(0).get(0)));// 设置显示数据
        }
        this.wv_option3.setCurrentItem(this.wv_option3.getCurrentItem());// 初始化时显示的数据
        int textSize = 25;
        ...
        this.wheelListener_option1 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (WheelOptions.this.mOptions2Items != null) {
                    opt2Select = WheelOptions.this.wv_option2.getCurrentItem();//上一个opt2的选中位置
                    //新opt2的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    opt2Select = opt2Select >= WheelOptions.this.mOptions2Items.get(index).size() - 1
                            ? WheelOptions.this.mOptions2Items.get(index).size() - 1 : opt2Select;
                    WheelOptions.this.wv_option2
                            .setAdapter(new ArrayWheelAdapter(WheelOptions.this.mOptions2Items.get(index)));
        ...
        this.wheelListener_option2 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                if (WheelOptions.this.mOptions3Items != null) {
                    ...
                    opt1Select = opt1Select >= WheelOptions.this.mOptions3Items.size() - 1
                            ? WheelOptions.this.mOptions3Items.size() - 1 : opt1Select;
                    index = index >= WheelOptions.this.mOptions2Items.get(opt1Select).size() - 1
                            ? WheelOptions.this.mOptions2Items.get(opt1Select).size() - 1 : index;
                    int opt3 = WheelOptions.this.wv_option3.getCurrentItem();//上一个opt3的选中位置
                    //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    opt3 = opt3 >= WheelOptions.this.mOptions3Items.get(opt1Select).get(index).size() - 1
                            ? WheelOptions.this.mOptions3Items.get(opt1Select).get(index).size() - 1 : opt3;
                    WheelOptions.this.wv_option3.setAdapter(new ArrayWheelAdapter(WheelOptions.this.mOptions3Items
                            .get(WheelOptions.this.wv_option1.getCurrentItem()).get(index)));
    ...
    private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
        if (this.mOptions2Items != null) {
            this.wv_option2.setAdapter(new ArrayWheelAdapter(this.mOptions2Items.get(opt1Select)));
            this.wv_option2.setCurrentItem(opt2Select);
        }
        if (this.mOptions3Items != null) {
            this.wv_option3.setAdapter(new ArrayWheelAdapter(this.mOptions3Items.get(opt1Select).get(opt2Select)));
