...
import com.benefit.buy.library.http.query.util.BitmapCache;
import com.benefit.buy.library.http.query.util.Common;
import com.benefit.buy.library.http.query.util.Constants;
import com.benefit.buy.library.http.query.util.RatioDrawable;
...
public class BitmapAjaxCallback extends AbstractAjaxCallback<Bitmap, BitmapAjaxCallback> {
    ...
    /**
     * Set the preset bitmap. This bitmap will be shown immediately until the ajax callback returns the final image from
     * the url.
    ...
    /**
     * Set the image aspect ratio anchor. Value of 1 implies show top end of the image, 0 implies at the center, -1
     * implies show at the bottom. A special value AQuery.ANCHOR_DYNAMIC will adjust the anchor base. This setting will
     * add up from 0 to 0.5 bias and it's suitable for portraits and common photos. Default value is ANCHOR_DYNAMIC.
    ...
    /**
     * Set the round corner radius. Note that the current implementation transform the image to a new one and will use
    ...
    public Bitmap transform(String url, byte[] data, AjaxStatus status) {
        ...
        if (bm == null) {
            ...
            else if ((fallback == Constants.GONE) || (fallback == Constants.INVISIBLE)) {
                bm = dummy;
            }
            else if (fallback == Constants.PRESET) {
                bm = preset;
            }
    ...
    /**
     * Sets the file cache write policy. If set to true, images load from network will be served quicker before caching
     * to disk, this however increase the chance of out of memory due to memory allocation. Default is false.
     * @param limit the new cache limit
     */
    ...
    /**
     * Sets the pixel criteria for small images. Small images are cached in a separate cache. Default is 50x50 (2500
     * pixels)
    ...
    private static Bitmap filter(View iv, Bitmap bm, int fallback) {
        ...
        else if (fallback == Constants.GONE) {
            iv.setVisibility(View.GONE);
        }
        else if (fallback == Constants.INVISIBLE) {
    ...
    private static void setBmAnimate(ImageView iv, Bitmap bm, Bitmap preset, int fallback, int animation, float ratio,
            float anchor, int source) {
    ...
    private static boolean fadeIn(int animation, int source) {
        switch (animation) {
            case Constants.FADE_IN:
                return true;
            case Constants.FADE_IN_FILE:
                if (source == AjaxStatus.FILE) {
                    return true;
                }
            case Constants.FADE_IN_NETWORK:
    ...
    /**
     * AQuery internal use only. Please uses AQuery image() methods instead. Optimize memory usage if mem hit and
     * there's no custom callback.
     */
    public static void async(Activity act, Context context, ImageView iv, String url, Object progress, AccountHandle ah,
            ImageOptions options, HttpHost proxy, String networkUrl) {
        async(act, context, iv, url, options.memCache, options.fileCache, options.targetWidth, options.fallback,
                options.preset, options.animation, options.ratio, options.anchor, progress, ah, options.policy,
                options.round, proxy, networkUrl);
    }
     ...
     * there's no custom callback.
     */
    public static void async(Activity act, Context context, ImageView iv, String url, boolean memCache,
            boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animation, float ratio, float anchor,
            Object progress, AccountHandle ah, int policy, int round, HttpHost proxy, String networkUrl) {
        Bitmap bm = null;
        if (memCache) {
        ...
        if (bm != null) {
            ...
            cb.url(url).imageView(iv).memCache(memCache).fileCache(fileCache).targetWidth(targetWidth)
                    .fallback(fallbackId).preset(preset).animation(animation).ratio(ratio).anchor(anchor)
                    .progress(progress).auth(ah).policy(policy).round(round).networkUrl(networkUrl);
            if (proxy != null) {
