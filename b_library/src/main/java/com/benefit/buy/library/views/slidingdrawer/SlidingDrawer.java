 ...
 * SlidingDrawer can be used vertically or horizontally. A special widget composed of two children views: the handle,
 * that the users drags, and the content, attached to the handle and dragged with it. SlidingDrawer should be used as an
 * overlay inside layouts. This means SlidingDrawer should only be used inside of a FrameLayout or a RelativeLayout for
 * instance. The size of the SlidingDrawer defines how much space the content will occupy once slid out so SlidingDrawer
 * should usually use match_parent for both its dimensions. Inside an XML layout, SlidingDrawer must define the id of
 * the handle and of the content:
 * 
 ...
 * @see <a href="http://http://developer.android.com/reference/android/widget/SlidingDrawer.html"> SlidingDrawer</a>
 * @author Igor Morais
...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(final Context context, final AttributeSet attributeSet, final int defStyle) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attributeSet, R.styleable.SlidingDrawer, defStyle,
                0);
    ...
    protected void onLayout(final boolean changed, final int left, final int top, final int right, final int bottom) {
        if (!isTracking) {
            ...
            if (isVertical) {
                ...
                contentView.layout(0, topOffset + handleMeasuredHeight, contentView.getMeasuredWidth(), topOffset
                        + handleMeasuredHeight + contentView.getMeasuredHeight());
            }
            else {
                handleLeft = isExpanded ? topOffset : (width - handleMeasuredWidth) + bottomOffset;
                handleTop = (height - handleMeasuredHeight) / 2;
                contentView.layout(topOffset + handleMeasuredWidth, 0,
                        topOffset + handleMeasuredWidth + contentView.getMeasuredWidth(),
                        contentView.getMeasuredHeight());
            }
            handleView
                    .layout(handleLeft, handleTop, handleLeft + handleMeasuredWidth, handleTop + handleMeasuredHeight);
    ...
    protected void dispatchDraw(final Canvas canvas) {
        ...
        if (isTracking || isAnimating) {
            ...
            if (bitmap == null) {
                canvas.save();
                canvas.translate(isVertical ? 0 : handleView.getLeft() - topOffset, isVertical ? handleView.getTop()
                        - topOffset : 0);
                    ...
                    if (Math.abs(velocity) < maximumTapVelocity) {
                        if (isVertical ? (isExpanded && (top < (tapThreshold + topOffset)))
                                ...
                                : (isExpanded && (left < (tapThreshold + topOffset)))
                                        || (!isExpanded && (left > ((bottomOffset + getRight()) - getLeft()
                                                - handleWidth - tapThreshold)))) {
                            if (allowSingleTap) {
        ...
        if (!isAnimating) {
            if (contentView.isLayoutRequested()) {
                if (isVertical) {
                    ...
                    contentView.measure(MeasureSpec.makeMeasureSpec(getRight() - getLeft(), MeasureSpec.EXACTLY),
                            MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));
                    contentView.layout(0, topOffset + handleHeight, contentView.getMeasuredWidth(), topOffset
                            + handleHeight + contentView.getMeasuredHeight());
                }
                else {
                    ...
                    contentView.layout(handleWidth + topOffset, 0,
                            topOffset + handleWidth + contentView.getMeasuredWidth(), contentView.getMeasuredHeight());
                }
            }
    ...
    private void performFling(final int position, final float velocity, final boolean always) {
        ...
        if (isExpanded) {
            if (always
                    || ((velocity > maximumMajorVelocity) || ((position > (topOffset + (isVertical ? handleHeight
                            : handleWidth))) && (velocity > -maximumMajorVelocity)))) {
                animatedAcceleration = maximumAcceleration;
                if (velocity < 0) {
                    animatedVelocity = 0;
                }
            ...
            if (!always
                    && ((velocity > maximumMajorVelocity) || ((position > ((isVertical ? getHeight() : getWidth()) / 2)) && (velocity > -maximumMajorVelocity)))) {
                animatedAcceleration = maximumAcceleration;
                if (velocity < 0) {
                    animatedVelocity = 0;
                }
    ...
    private void moveHandle(final int position) {
        if (isVertical) {
            if (position == DRAWER_EXPANDED) {
                ...
                invalidateRect
                        .union(frameRect.left, frameRect.top - deltaY, frameRect.right, frameRect.bottom - deltaY);
            ...
            if (position == DRAWER_EXPANDED) {
                ...
                invalidateRect.set(frameRect);
                invalidateRect
                        .union(frameRect.left - deltaX, frameRect.top, frameRect.right - deltaX, frameRect.bottom);
                invalidateRect.union(frameRect.right - deltaX, 0, (frameRect.right - deltaX) + contentView.getWidth(),
                        getHeight());
                invalidate(invalidateRect);
            }
        }
